...
public static class Data
{
    ...
    // 각 테트로미노의 초기 상대 셀 위치 설정 (각 테트로미노의 모양)
    ...
    /* 테트로미노의 WallKick 테이블
     * WallKick은 테트로미노가 벽에 닿았을 때 회전을 조정하여 적절한 위치로 이동시킴
     * WallKick이 없으면 벽에 붙어서 회전할 때 블록이 벽을 뚫는 현상이 발생함 */

    // I 테트로미노의 WallKick 테이블 ( [,] -> 이차원 배열 )
    ...
    // 나머지 테트로미노(JLOSTZ)의 WallKick 테이블 ( [,] -> 이차원 배열 )
    ...
    // 각 테트로미노에 대한 WallKick 테이블 딕셔너리 ( [,] -> 이차원 배열 )
